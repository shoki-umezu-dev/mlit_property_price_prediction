...
### ステップ特徴量
ステップ特徴量とは、**時間インデックスから直接導出できる特徴量である**  
例: 時系列データの開始から終了までの時間ステップをカウントするダミーデータ  
時間をx軸、目的変数をy軸と置いて線形回帰を行うことで、典型的な時系列データの線形予測が成り立つ  

### タイムステップ機能
タイムステップ機能は**時間依存性**をモデル化できる  
その値が**出現した時点から予測できる場合**に、時間依存性を持つと言える  
例: ハードカバー販売データの場合、月の後半の売上は月の前半の売り上げよりも一般的に高くなる  

### ラグ機能
対象系列の観測値を時間的にずらしたように見せたもの  
例: ハードカバーの販売データの場合、その月の売上と1つ前の月の売り上げを並べて比較すること  

ラグ特徴量を用いることで、時系列内の各観測値と単位分ずれた観測値との関係性を可視化できる  

### トレンド
その系列の平均値における**持続的、長期的な変化**を表す  
系列の中で最も動きが遅く、最も重要な時間スケール  

移動平均プロット: 時系列データの傾向を把握するために、一定の幅で平均を取る操作を時間軸に沿って行う手法
...
## 特徴量エンジニアリングすべき項目
- transaction_price / 平米数の1~5年までのラグ(エリアで分けてから)
- transaction_yearとtransaciton_quarterの1~5年までのラグ(エリアで分けてから)
- 大阪万博の宣伝、インフラ開発など、イベントごとのフラグ作成
- 大阪万博の最寄り駅、乗り換え回数、駅徒歩+最寄り駅間での所要時間
- その年(ないしは四半期)ごとの外国人宿泊者数を別データから引っ張ってくる
- 外国人世帯数の推移を大阪市オープンデータポータルサイトから持ってくる
- 
